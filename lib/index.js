...
(function () {
    ...
    var Flow = comb.define(EventEmitter, {
        ...
        instance:{
            ...
            retract:function (fact, throwIfMissing) {
                //fact = this.workingMemory.getFact(fact);
                this.__wmAltered = true;
                this.__factHelper(fact, false);
                this.emit("retract", fact, throwIfMissing);
                return fact;
            },

            // This method is called to alter an existing fact.  It is essentially a
            // retract followed by an assert.
            modify:function (fact, cb, throwIfMissing) {
                //fact = this.workingMemory.getFact(fact);
                this.retract(fact, throwIfMissing);
            ...
            __factHelper:function (object, assert, throwIfMissing) {
                var f = new Fact(object);
                if (assert) {
                    this.__assertFact(f);
                } else {
                    this.__retractFact(f, throwIfMissing);
                }
                return f;
            },
            ...
            __retractFact:function (fact, throwIfMissing) {
                var wmFact = this.workingMemory.retractFact(fact, throwIfMissing);
                if (wmFact && this.rootNode) {
                    this.rootNode.retractFact(wmFact, throwIfMissing);
                }
            }
        }
    });
